...
Error
Mesh2D::read(const int mpi_rank) {
  ...
  ra_netcdf_invoke(nc_inq_dimid(file, "extent.1", &(dimension.extent[1])));
  ...
  const std::size_t length_f =
    geometry.element.dof * (geometry.extent[0] * geometry.extent[1]);
