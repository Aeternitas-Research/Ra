...
test_sub2ind_right_host(
  ...
  for (int index = 0; index < N; ++index) {
    ...
    output[index] =
      ra::sub2ind_right(input.data() + offset, extent.data(), dimension);
    if (dimension == 1) {
      REQUIRE(output[index] == input[offset + 0]);
    } else if (dimension == 2) {
      REQUIRE(
        output[index] == (input[offset + 0] * extent[1] + input[offset + 1]));
    } else if (dimension == 3) {
      REQUIRE(
        output[index] ==
        ((input[offset + 0] * extent[1] + input[offset + 1]) * extent[2] +
         input[offset + 2]));
    } else if (dimension == 4) {
      REQUIRE(
        output[index] ==
...
TEST_CASE("sub2ind_right", "[index]") {
  ...
  for (int dimension = 1; dimension <= 6; ++dimension) {
    ...
    test_sub2ind_right_host(host.output, host.input, host.extent, dimension);
    test_sub2ind_right_device(
