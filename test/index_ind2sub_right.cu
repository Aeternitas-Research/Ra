...
test_ind2sub_right_host(
  ...
  for (int index = 0; index < N; ++index) {
    ...
    input[index] =
      ra::sub2ind_right(output.data() + offset, extent.data(), dimension);
  }
  ...
  for (int index = 0; index < N; ++index) {
    ...
    if (dimension == 1) {
      REQUIRE(output[offset + 0] == input[index]);
    } else if (dimension == 2) {
      REQUIRE(output[offset + 0] == (input[index] / extent[1]) % extent[0]);
      REQUIRE(output[offset + 1] == input[index] % extent[1]);
    } else if (dimension == 3) {
      REQUIRE(output[offset + 0] == (input[index] / extent[1]) % extent[0]);
      REQUIRE(output[offset + 1] == (input[index] / extent[2]) % extent[1]);
      REQUIRE(output[offset + 2] == input[index] % extent[2]);
    } else if (dimension == 4) {
      REQUIRE(output[offset + 0] == (input[index] / extent[1]) % extent[0]);
      REQUIRE(output[offset + 1] == (input[index] / extent[2]) % extent[1]);
      REQUIRE(output[offset + 2] == (input[index] / extent[3]) % extent[2]);
      REQUIRE(output[offset + 3] == input[index] % extent[3]);
    } else if (dimension == 5) {
      REQUIRE(output[offset + 0] == (input[index] / extent[1]) % extent[0]);
      REQUIRE(output[offset + 1] == (input[index] / extent[2]) % extent[1]);
      REQUIRE(output[offset + 2] == (input[index] / extent[3]) % extent[2]);
      REQUIRE(output[offset + 3] == (input[index] / extent[4]) % extent[3]);
      REQUIRE(output[offset + 4] == input[index] % extent[4]);
    } else if (dimension == 6) {
      REQUIRE(output[offset + 0] == (input[index] / extent[1]) % extent[0]);
      REQUIRE(output[offset + 1] == (input[index] / extent[2]) % extent[1]);
      REQUIRE(output[offset + 2] == (input[index] / extent[3]) % extent[2]);
      REQUIRE(output[offset + 3] == (input[index] / extent[4]) % extent[3]);
      REQUIRE(output[offset + 4] == (input[index] / extent[5]) % extent[4]);
      REQUIRE(output[offset + 5] == input[index] % extent[5]);
    } else {
      REQUIRE(false);
    }
  }
...
template <typename T>
__global__ void
invoke_sub2ind_right_device(
  thrust::device_ptr<T> output, thrust::device_ptr<T> input,
  thrust::device_ptr<T> extent, const int dimension) {
  int index = threadIdx.x + blockIdx.x * blockDim.x;
  if (index < N) {
    output[index] =
      ra::sub2ind_right(input.get() + 6 * index, extent.get(), dimension);
  }
}

template <typename T>
__global__ void
invoke_ind2sub_right_device(
  thrust::device_ptr<T> output, thrust::device_ptr<T> input,
  thrust::device_ptr<T> extent, const int dimension) {
  int index = threadIdx.x + blockIdx.x * blockDim.x;
  if (index < N) {
...
test_ind2sub_right_device(
  ...
  invoke_sub2ind_right_device<<<N / 256, 256>>>(
    input.data(), output.data(), extent.data(), dimension);
  thrust::fill(output.begin(), output.end(), 0);
  invoke_ind2sub_right_device<<<N / 256, 256>>>(
...
TEST_CASE("ind2sub_right", "[index]") {
  ...
  for (int dimension = 1; dimension <= 6; ++dimension) {
    ...
    test_ind2sub_right_host(host.output, host.input, host.extent, dimension);
    test_ind2sub_right_device(
